#!/usr/bin/env python3
"""Generate the v3 ViennaRNA 2.7.2 GPU benchmark notebook.

v3 = v2 plus the two int16 arms (RNA_FML_INT16). v2 answered what chunking
costs; v3 answers what int16 buys, which is TWO independent things that
multiply, and it is the first measurement of either.
...
# --------------------------------------------------------------------------
md(r"""# CUDA_RNAFold on ViennaRNA 2.7.2 — benchmark v3
...
v2 established that **chunking costs roughly k× the wall clock for k chunks** —
not per-chunk overhead, but loss of batch width, because the sweep is row-serial
and a chunk costs (max record length) rows however many records it holds.

v3 measures `RNA_FML_INT16`, which is expected to pay **twice, independently**:

| | what it should buy | measured by |
|---|---|---|
| **the DRAM stream** | `fml_j` is the irreducible ~46.7 TB stream; halving it should cut the kernel's time | **C vs E** (both single-chunk) |
| **the chunk count** | 48.3% less VRAM per record ⇒ ~2× the records per chunk ⇒ ~half the chunks at the same budget | **D vs F** (same budget) |

Those multiply, and neither has ever been measured. The encoding is already
proven *correct* — byte-identical A/B on 176 records, and `RNA_ROW_VERIFY` at
17 941 560 cells with zero mismatches — but nothing yet proves it is *faster*.
...
| **INT16** | the same again with `RNA_FML_INT16=1`, which must give the *same answer* |
...
- **a "multi-chunk" arm with only ONE sweep** → the budget didn't bite and the
  arm tested nothing. Same shape as the trap where `RNA_GPU_CHUNK=8` against
  `MIN_GPU_BATCH=10` produced a green GPU bar that had never touched the GPU.
- **an "int16" arm where the gate never engaged** → it is just the int32 arm
  again wearing a different label, and would report a speedup of exactly 1.00x
  while looking like a real measurement. The run greps the sweep's own
  `RNA_FML_INT16=1` announcement to prove otherwise.
- clock throttling.""")
...
def run_once(binary, fa, gpu, budget_mb=None, int16=False):
    env = dict(os.environ)
    ...
    return dict(wall=wall, rss=rss, rc=p.returncode,
                ...
                # The gate announces itself on stderr. Without this an int16 arm
                # that silently ran as int32 would report 1.00x and look real.
                int16_active=("RNA_FML_INT16=1" in p.stderr),
                stderr_tail=p.stderr[-600:] if p.returncode else "")

# (config label, arm key, gpu?, int16?)
# E pairs with C (same budget: none) and F pairs with D (same budget), so each
# int16 arm has an int32 twin differing in exactly one variable.
CONFIGS = [
    ("A upstream",      "A upstream 2.7.2",   False, False),
    ("B port, no cuda", "B port27 (no cuda)", False, False),
    ("C GPU 1-chunk",   "C port27 (cuda)",    True,  False),
    ("D GPU N-chunk",   "C port27 (cuda)",    True,  False),
    ("E i16 1-chunk",   "C port27 (cuda)",    True,  True),
    ("F i16 N-chunk",   "C port27 (cuda)",    True,  True),
]
...
for wname, fa in W.items():
    ...
    budget_for_cfg = {}
    for label, armkey, gpu, i16 in CONFIGS:
        b = BIN[armkey]
        ...
        elif label == "F i16 N-chunk":
            # THE SAME budget as D, deliberately. That is the comparison: at a
            # fixed VRAM allowance, how many chunks does each encoding need?
            budget_for_cfg[label] = BUDGET[wname]
            p16 = run_once(b, fa, True, BUDGET[wname], int16=True)
            print(f"    int16 at the same {BUDGET[wname]} MB -> {p16['sweeps']} chunks"
                  f"  (int32 needed {results.get((wname,'D GPU N-chunk'),{}).get('sweeps','?')})")
        else:
            budget_for_cfg[label] = None
            if gpu:
                run_once(b, fa, gpu, None, int16=i16)   # warm-up, GPU only

    # Timed rounds, one rep of every arm per round.
    runs = {label: [] for label, _, _, _ in CONFIGS}
    for rep in range(REPS):
        for label, armkey, gpu, i16 in CONFIGS:
            runs[label].append(run_once(BIN[armkey], fa, gpu,
                                        budget_for_cfg[label], int16=i16))
        print(f"    round {rep+1}/{REPS}: " +
              "  ".join(f"{lab.split()[0]}={runs[lab][-1]['wall']:.1f}s"
                        for lab, _, _, _ in CONFIGS))
    ...
    for label, armkey, gpu, i16 in CONFIGS:
        ...
        results[(wname, label)] = dict(
            ...
            int16_active=r0["int16_active"], int16_wanted=i16,
...
code(r"""ok = True
for wname in W:
    shas = {lab: results[(wname, lab)]["sha"] for lab, _, _, _ in CONFIGS}
    if len(set(shas.values())) != 1:
        ok = False
        print(f"  {wname}: ARMS DISAGREE -- timings are NOT comparable")
        for a, s in shas.items(): print(f"      {a:18s} {s}")
    else:
        print(f"  {wname}: all six configs identical  ({list(shas.values())[0]})")
        print(f"      ^ 1-chunk vs N-chunk: chunk boundaries do not move the answer")
        print(f"      ^ int32 vs int16: the encoding is bit-exact at benchmark scale")
    ...
    # The int16 arms must have actually engaged the gate. An arm that silently
    # ran as int32 would report a 1.00x speedup and look like a real result.
    for lab in ("E i16 1-chunk", "F i16 N-chunk"):
        r = results[(wname, lab)]
        if not r["int16_active"]:
            ok = False
            print(f"  {wname}: {lab} never announced RNA_FML_INT16=1 -- it ran as"
                  f" int32, so any ratio against it is meaningless")
        if r["sweeps"] == 0:
            ok = False
            print(f"  {wname}: {lab} ran ZERO sweeps -- the budget dropped it below"
                  f" MIN_GPU_BATCH and it folded on the CPU, so its time is not a"
                  f" GPU time")

    # F is the whole point of F: at the SAME budget, int16 should need fewer
    # chunks. If it needs the same number, the VRAM saving did not translate.
    dch = results[(wname, "D GPU N-chunk")]["sweeps"]
    fch = results[(wname, "F i16 N-chunk")]["sweeps"]
    if fch >= dch:
        print(f"  {wname}: NOTE int16 needed {fch} chunks where int32 needed {dch}"
              f" at the same budget -- the VRAM saving did not reduce the chunk count")
    else:
        print(f"      int16 needs {fch} chunks where int32 needs {dch}, same budget")
...
code(r"""print(f"{'workload':16s} {'A upstream':>11s} {'B port,off':>11s} {'C 1-chunk':>11s}"
      f" {'E i16':>11s} {'A/C':>7s} {'A/E':>7s} {'A/B':>7s}")
for wname in W:
    a = results[(wname, "A upstream")]["best"]
    b = results[(wname, "B port, no cuda")]["best"]
    c = results[(wname, "C GPU 1-chunk")]["best"]
    e = results[(wname, "E i16 1-chunk")]["best"]
    print(f"{wname:16s} {a:10.2f}s {b:10.2f}s {c:10.2f}s {e:10.2f}s "
          f"{a/c:6.2f}x {a/e:6.2f}x {a/b:6.2f}x")

print()
print("--- what int16 buys, split into its two independent effects ---")
print(f"{'workload':16s} {'C int32':>9s} {'E int16':>9s} {'C/E':>7s} | "
      f"{'D int32':>9s} {'F int16':>9s} {'D/F':>7s} | {'chunks D->F':>12s}")
for wname in W:
    c = results[(wname, "C GPU 1-chunk")]
    e = results[(wname, "E i16 1-chunk")]
    d = results[(wname, "D GPU N-chunk")]
    f_ = results[(wname, "F i16 N-chunk")]
    print(f"{wname:16s} {c['best']:8.2f}s {e['best']:8.2f}s {c['best']/e['best']:6.2f}x | "
          f"{d['best']:8.2f}s {f_['best']:8.2f}s {d['best']/f_['best']:6.2f}x | "
          f"{d['sweeps']:5d} -> {f_['sweeps']:-4d}")
print("C/E  the DRAM-stream win alone: both single-chunk, so chunk count is held")
print("     fixed and the only difference is the width of the fml_j stream.")
print("D/F  the win at a FIXED VRAM budget, where the halved footprint also buys")
print("     fewer chunks. v2 measured chunking at 2.3-4.2x, so this is where the")
print("     second effect shows up -- and the two multiply.")
print()
print("Note that chunk counts are INTEGERS, so this effect is quantised: measured")
print("locally, a 60x900 nt workload needed 4 chunks at int32 and 3 at int16 on a")
print("48 MB budget, but 3 and 3 on 64 MB. Tighten the budget far enough and the")
print("difference stops being a ratio at all -- at 32 MB the int32 arm fell below")
print("MIN_GPU_BATCH and folded entirely on the CPU while int16 still ran on the")
print("GPU. So D/F understates the benefit on a card that is genuinely too small.")
print("The scope doc predicted ~0.52x on the ~73%% of wall that is")
print("modular_decomposition, i.e. C/E around 1.4x, and A/E near 8x at 5601 nt")
print("where A/C was 5.65x. That prediction falls out of the model rather than")
print("being fitted, so a miss is informative.")
...
print()
print(f"{'workload':16s} {'peak RSS A':>12s} {'B':>8s} {'C':>8s} {'D':>8s} {'E':>8s} {'F':>8s}")
for wname in W:
    print(f"{wname:16s} "
          ...
          f"{results[(wname,'E i16 1-chunk')]['rss']:7.2f}G "
          f"{results[(wname,'F i16 N-chunk')]['rss']:7.2f}G")""")
...
with open("/content/bench272_v3.json", "w") as f:
    json.dump(out, f, indent=2)
print(json.dumps(out, indent=2)[:2000])
try:
    from google.colab import files
    files.download("/content/bench272_v3.json")
...
with open("CUDA_RNAFold_Bench272_v3.ipynb", "w") as f:
    json.dump(nb, f, indent=1)
print("wrote CUDA_RNAFold_Bench272_v3.ipynb", len(C), "cells")
