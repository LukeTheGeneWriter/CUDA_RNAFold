...
AC_DEFUN([RNA_ENABLE_CUDA],[
  ...
  RNA_ADD_FEATURE([cuda],
                  [CUDA GPU computation support],
                  [yes])

  AC_ARG_VAR([NVCC_PATH],[Path to the CUDA compiler, e.g. /opt/cuda/bin])
  ...
  RNA_FEATURE_IF_ENABLED([cuda],[
    AC_PATH_PROG([NVCC_BIN], [nvcc], [no], ["${NVCC_PATH}":$PATH])
    AC_CHECK_PROG([NVCC_HOST_CC], [$NVCC_HOST_CC], [$CXX])

    if test "x${NVCC_BIN}" = xno;
    then
