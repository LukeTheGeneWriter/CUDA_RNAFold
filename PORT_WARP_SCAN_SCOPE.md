...
| **H5 cut registers to raise occupancy** | **DEAD — now measured, not just derived.** NCU on the A100 reports the limiters as blocks 32 / registers 40 / smem 32 / warps 64: the 32-blocks-per-SM *hardware* cap binds and registers have slack. §30.3. Originally **DEAD ON ARITHMETIC, §28.1** — at the shipped one-warp-per-block the ceiling is 32 warps and it is the 32-blocks-per-SM *hardware* limit, not registers; driving registers to zero would not move it. The occupancy figure it was ranked on was also confounded by grid size (§28.2). *Superseded reasoning:* **NEW and now first** — registers are the *measured* binding limiter at 58/thread, achieved occupancy 20.7 %, and `long_scoreboard` (what more warps would hide) is **44.4 % of stalls**. §27.6 |
| **H6 2-D grid: `blockIdx.y` is the record** | **CONFIRMED ON A100: −5.65 %** (sm_86 said −7.5 %), control flat to 0.07 %, 2-D grid proven taken. §30.1. Previously: **BUILT, −7.6 %, byte-identical, gated off** (`RNA_INT_LOOP_GRIDY=1`) pending an A100 arm. Deletes the nine-deep chain of *dependent* global loads every warp runs before it can start — `flatten_index_to_H()`'s binary search. §28.4 |

| **H7 32-ary warp-cooperative lookup** | **CONFIRMED ON A100: −3.6 %** (sm_86 said −3.7 %) and **the only thing that helps ragged input**, where H6 declines every launch and delivers −0.15 %. §30.2. (`RNA_INT_LOOP_WSEARCH=1`). Keeps the flat grid and makes the search `log32` instead of `log2` — so it applies where H6's waste guard DECLINES, which is every ragged chunk and every row continuous flow has retired a record in. §29 |
